...
from .files import ACR2FULL, get_full_instruction
...
def get_subdomain_tokens(rubrics, encoding):
    ...
    """
    subdomain_tokens = {}
    for subdomain, rubric_content in rubrics.items():
        # Count tokens in the rubric content
        subdomain_tokens[subdomain] = {
            'full_name': len(encoding.encode(ACR2FULL[subdomain])),
            'rubric': len(encoding.encode(rubric_content))
        }
...
def get_total_tokens_aprox(prompts, rubrics, max_completion_tokens, encoding):
    """
    ...
    empty_instuction = get_full_instruction('', ' ', '', max_completion_tokens)
...
def get_total_tokens_exact_serial(prompts, rubrics, max_completion_tokens, encoding):
    """
    ...
    total_input_tokens = 0
    for subdomain, rubric_content in rubrics.items():
        for prompt in tqdm(prompts, total=len(prompts), desc=f"Tokenizing subdomain {subdomain}"):
            full_prompt = get_full_instruction(ACR2FULL[subdomain], rubric_content, prompt, max_completion_tokens)
            total_input_tokens += get_instruction_tokens(full_prompt, encoding)
    return total_input_tokens


def get_total_tokens_exact(prompts, rubrics, max_completion_tokens, encoding, max_workers=-1):
    """
    Calculate the exact total number of tokens for the given prompt data and rubrics.
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = []
        for subdomain, rubric_content in rubrics.items():
            futures.append(executor.submit(subdomain_token_sum, subdomain, rubric_content))
...
def get_batch_api_pricing(
    ...
    Args:
        ...
        n_jobs (int or None): Number of parallel jobs for exact estimation. Default is -1 (all available cores).

    Returns:
        float: Estimated cost in USD.
    """
    # Check if the model is valid
    if openai_model not in MODEL2PRICE:
        raise ValueError(f"Invalid OpenAI model: {openai_model}. Supported models are: {', '.join(MODEL2PRICE.keys())}")
    
    encoding = get_encoding(openai_model)

    # Calculate input cost
    if estimation == 'aprox':
        total_input_tokens = get_total_tokens_aprox(prompts, rubrics, max_completion_tokens, encoding)
    elif estimation == 'exact':
        if n_jobs is None:
            max_workers = -1
        else:
            max_workers = resolve_n_jobs(n_jobs, verbose=False)
        total_input_tokens = get_total_tokens_exact(prompts, rubrics, max_completion_tokens, encoding, max_workers=max_workers)
    else:
        raise ValueError("Invalid estimation type. Use 'aprox' or 'exact'.")
    cost_per_1M_input_tokens = MODEL2PRICE[openai_model]['input']
    estimated_input_cost = (total_input_tokens / 1000000) * cost_per_1M_input_tokens

    # Calculate output cost
    total_output_tokens = len(prompts) * len(rubrics) * max_completion_tokens
    cost_per_1M_output_tokens = MODEL2PRICE[openai_model]['output']
    estimated_output_cost = (total_output_tokens / 1000000) * cost_per_1M_output_tokens
